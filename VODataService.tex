...
\paragraph{Find resources by the sort of dataset they serve.}
Researchers are looking for, say, time series (as opposed to spectra or
images) and only want to discover resources serving these.  The implicit
dataset typing by service type (``discover spectra by a global SSAP
query'') that has been in use since the beginning of the VO no longer
good enough since the introduction of polyvalent protocols like Obscore.
Hence, VODataService 1.3 has a new element \xmlel{productTypeServed}.

\paragraph{Separate theory from observation.}  Researchers want to
exclude data from simulations from their discovery, where ``simulation''
might be a digital twin of an instrument doing an observation of a model
of an actual object on the other hand, or completely artificial data
starting from random numbers on the other.  Or they might look for mock
data that may be available for a given instrument setup, or for
simulation data to match with their observations.  VODataService 1.3
accomodates these use cases by introducing a \xmlel{dataSource} element.
...
The \xmlel{vs:DataResource} resource base type can be used to describe a
resource containing generic astronomical data.
An example might be a largely unstructured archive
...
astronomical catalogues) should use the derived type \xmlel{vs:CatalogResource}.

\xmlel{DataResource} is derived from \xmlel{vr:Service}, which means
that instances can have
capabilities.  For plain \xmlel{vs:DataResource}, only auxiliary capabilities
...
In addition, two metadata elements giving tokens from controlled
vocabularies are introduced by DataResource.  With
\xmlel{productTypeServed}, resources say which data product types (if
any) they serve.  This is intended to replace the practice of doing data
discovery based on service types (e.g., ``search all SIAP services if
you are looking for images'') that was dominant in the VO from its
beginnings and became inappropriate as TAP-published Obscore tables
\citep{2017ivoa.spec.0509L} came into widespread use.

While a flat list is not an adequate presentation for this fairly
hierarchical vocabulary, here is the list of terms contained in
\url{http://www.ivoa.net/rdf/product-type} at the time of writing:

% GENERATED: !vocterms product-type
\textsl{cube},
\textsl{dynamic-spectrum},
\textsl{event-bundle},
\textsl{event-list},
\textsl{image},
\textsl{light-curve},
\textsl{measurements},
\textsl{polarization-cube},
\textsl{polarization-resolved-dataset},
\textsl{polarized-spectrum},
\textsl{sed},
\textsl{slit-spectrum},
\textsl{spatial-profile},
\textsl{spatially-resolved-dataset},
\textsl{spectral-cube},
\textsl{spectrally-resolved-dataset},
\textsl{spectrum},
\textsl{temporally-resolved-dataset},
\textsl{time-cube},
\textsl{timeseries},
\textsl{velocity-curve},
\textsl{visibility}
% /GENERATED

See \citet{2023ivoa.spec.0206D} for how to use IVOA vocabularies more
appropriately.

In the other vocabulary-controlled element, \xmlel{dataSource},
publishers can declare whether the data published was the result of an
observation or of some theoretical work.  The central purpose here is to
enable filtering out data not actually matching any physical object up
front.  At this point, the controlled vocabulary
...
assumed to publish data resulting from observations.

Consumers should not rely on the term lists given here but retrieve the
latest version of the pertinent vocabularies and exploit their semantic
relationships (e.g., expanding to narrower terms when comparing for
equality) when sensible.
...
VODataService uses the string serialisation defined in section 4.3.2 of
\citet{2022ivoa.spec.0727F}.
...
% GENERATED: !schemadoc VODataService-v1.3.xsd SpatialCoverage
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd FloatInterval
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd ServiceReference
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
The \xmlel{vs:TableSet} type should be used
to describe the set of tables that are part of a single resource.
While there is
...
% GENERATED: !schemadoc VODataService-v1.3.xsd TableSet
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
The order in which \xmlel{schema} elements are given within a tableset
is significant.  Publishers should put schemas they consider more
important or interesting lexically before less salient ones.  For TAP
services, this order should be consistent with the one implied by
TAP\_SCHEMA's \verb|schema_index|.


% GENERATED: !schemadoc VODataService-v1.3.xsd TableSchema
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
\xmlel{vs:Table} type.
As with \xmlel{schema}, the order of the \xmlel{table} elements is
significant, with more salient tables listed further up within a schema
element.  For TAP services, this order should be consistent with the one
implied by TAP\_SCHEMA's \verb|table_index|.



% GENERATED: !schemadoc VODataService-v1.3.xsd Table
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd ForeignKey
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd ParamHTTP
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd BaseParam
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd DataType
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd SimpleDataType
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd InputParam
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
% GENERATED: !schemadoc VODataService-v1.3.xsd TableParam
\begin{generated}
\begingroup
        \renewcommand*\descriptionlabel[1]{%
...
\begin{itemize}
\item Adding a \xmlel{stats} child to \xmlel{BaseParam} to declare
basic column statistics.
\item Adding a \xmlel{dataSource} child to \xmlel{DataService} in order
to distinguish observed and theoretical data.
\end{itemize}
...
1.4.
\item Clarifying that the order of schema and table elements in a
tableset is significant.
\end{itemize}
